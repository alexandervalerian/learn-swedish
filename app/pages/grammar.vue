...
function markStudied(id: string) { userStore.markStudied(id) }
function markPracticed(id: string) { userStore.markPracticed(id) }
...
function handleBack() {
  if (phase.value === 'topic') { phase.value = 'list'; return }
  if (phase.value === 'list' && levelContext.value) { router.push(backTarget.value); return }
  if (phase.value === 'list') { phase.value = 'picker'; return }
  router.push('/')
}
...
function submitExercise() {
  ...
  const accepted = [currentExercise.value.answer, ...(currentExercise.value.alternatives ?? [])]
...
<template>
  <div class="max-w-lg mx-auto px-4 pt-6 pb-24">
    ...
    <div class="flex items-center gap-3 mb-6">
      <button class="page-back-btn" @click="handleBack">
        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
        </svg>
      </button>
      <div class="flex-1 min-w-0">
        <h2 class="font-bold text-ink-primary">Grammatik</h2>
        <p class="text-xs text-ink-tertiary mt-0.5 truncate">
          <template v-if="phase === 'picker'">Niveau wählen</template>
          <template v-else-if="phase === 'list'">
            <button class="hover:text-brand transition-colors" @click="phase = 'picker'">Grammatik</button>
            <span class="mx-1">›</span>
            <span class="text-ink-secondary">{{ allLevels.find(l => l.label === selectedLevel)?.emoji }} {{ allLevels.find(l => l.label === selectedLevel)?.fullLabel }}</span>
          </template>
          <template v-else>
            <button class="hover:text-brand transition-colors" @click="phase = 'picker'">Grammatik</button>
            <span class="mx-1">›</span>
            <button class="hover:text-brand transition-colors" @click="phase = 'list'">{{ allLevels.find(l => l.label === selectedLevel)?.emoji }} {{ allLevels.find(l => l.label === selectedLevel)?.fullLabel }}</button>
            <span class="mx-1">›</span>
            <span class="text-ink-secondary truncate">{{ selectedTopic?.title }}</span>
          </template>
        </p>
      </div>
    </div>
    ...
    <div v-if="phase === 'picker'">
      <p class="text-sm text-ink-tertiary mb-6">Lerne Schwedische Grammatik Schritt für Schritt – von den Grundlagen bis zum fortgeschrittenen Niveau.</p>
      <div class="space-y-3">
        <button
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <span class="text-sm font-medium text-ink-secondary">{{ lvl.data.topics.length }} Grammatikthemen</span>
          </div>
      ...
      <div v-for="[category, topics] in topicsByCategory" :key="category" class="mb-5">
        ...
        <div class="card overflow-hidden divide-y divide-surface-inset">
          <button
            ...
            :key="topic.id"
            class="w-full flex items-center gap-3 px-4 py-3 text-left transition-all active:scale-[0.99] hover:bg-surface-inset/50"
            @click="openTopic(topic)"
          >
            <div class="flex-1 min-w-0">
              <p class="text-sm font-semibold text-ink-primary">{{ topic.title }}</p>
              <p class="text-xs text-ink-tertiary mt-0.5">{{ topic.exercises.length }} Übungen</p>
            </div>
            <div class="flex items-center gap-1.5 flex-shrink-0">
              <span
                v-if="userStore.grammarProgress.practiced.includes(topic.id)"
                class="text-[10px] font-bold text-correct"
              >Geübt ✓</span>
              <span
    ...
    <div v-else-if="phase === 'topic' && selectedTopic">
      <h3 class="text-lg font-bold text-ink-primary mb-4">{{ selectedTopic.title }}</h3>

      <!-- Tabs -->
      <div class="flex gap-1 mb-6 bg-surface-inset rounded-xl p-1">
        <button
          class="flex-1 py-2 rounded-lg text-sm font-semibold transition-all"
          :class="activeTab === 'erklaerung' ? 'bg-white text-ink-primary shadow-sm' : 'text-ink-tertiary'"
        ...
        <button
          class="flex-1 py-2 rounded-lg text-sm font-semibold transition-all"
          :class="activeTab === 'uebungen' ? 'bg-white text-ink-primary shadow-sm' : 'text-ink-tertiary'"
          @click="goToUebungen"
        >
          Übungen
        </button>
      ...
      <div v-if="activeTab === 'erklaerung'">
        <!-- Rule box — brand-tinted -->
        <div class="rounded-xl bg-brand-subtle border border-brand-muted px-4 py-3 mb-4">
          <p class="text-[10px] font-bold text-brand uppercase tracking-widest mb-1">Regel</p>
          <pre class="text-sm text-brand-deeper whitespace-pre-wrap font-mono leading-relaxed">{{ selectedTopic.rule }}</pre>
        </div>

        <!-- Explanation -->
        <p class="text-sm text-ink-secondary leading-relaxed mb-6">{{ selectedTopic.explanation }}</p>

        <!-- Examples -->
        <div class="mb-6">
          <p class="section-label">Beispiele</p>
          <div class="space-y-2">
            <div
              v-for="(ex, i) in selectedTopic.examples"
              :key="i"
              class="border-l-[3px] border-brand pl-4 py-2"
            >
              <p class="text-base font-medium text-ink-primary mb-0.5">
                <template v-if="highlightSentence(ex.swedish, ex.highlight)">
                  <span>{{ highlightSentence(ex.swedish, ex.highlight)!.before }}</span>
                  <span class="bg-gold-soft text-ink-primary rounded px-0.5 font-semibold">{{ highlightSentence(ex.swedish, ex.highlight)!.highlighted }}</span>
                  <span>{{ highlightSentence(ex.swedish, ex.highlight)!.after }}</span>
                </template>
                <template v-else>{{ ex.swedish }}</template>
              </p>
              <p class="text-sm text-ink-tertiary italic">{{ ex.german }}</p>
            </div>
          </div>
        </div>

        <!-- CTA -->
        <button class="btn-primary" @click="goToUebungen">
          Übungen starten →
        </button>
      </div>
      ...
      <div v-else-if="activeTab === 'uebungen'">
        ...
        <div v-if="exerciseDone" class="text-center py-8">
          <p class="text-5xl font-black mb-2" :class="exerciseCorrectCount === exerciseResults.length ? 'text-correct' : 'text-brand'">
          ...
          <div class="flex flex-col gap-3">
            <button class="btn-primary" @click="startExercises">Nochmal üben</button>
            <button class="btn-secondary" @click="activeTab = 'erklaerung'">Zurück zur Erklärung</button>
          </div>
        </div>
        ...
        <template v-else-if="currentExercise">
          ...
          <p class="text-xs text-ink-tertiary mb-4">Übung {{ exerciseIndex + 1 }} von {{ selectedTopic.exercises.length }}</p>

          <p class="text-lg font-medium text-ink-primary mb-2 leading-relaxed">{{ currentExercise.prompt }}</p>
          <p class="text-sm text-ink-tertiary italic mb-5">{{ currentExercise.translation }}</p>
          ...
          <div class="space-y-3">
            <input
              ...
              :class="exerciseSubmitted
                ? exerciseCorrect ? 'border-correct-border bg-correct-bg' : 'border-wrong-border bg-wrong-bg'
          ...
          <Transition name="slide-up">
            <div
              ...
              :class="exerciseCorrect ? 'bg-correct-bg border border-correct-border' : 'bg-wrong-bg border border-wrong-border'"
            >
              <div class="flex items-start gap-2">
                <span class="text-lg mt-0.5">{{ exerciseCorrect ? '✓' : '✗' }}</span>
                <div>
                  <p class="font-semibold text-sm" :class="exerciseCorrect ? 'text-correct' : 'text-wrong'">
                    {{ exerciseCorrect ? 'Richtig!' : 'Falsch' }}
                  </p>
                  <p v-if="!exerciseCorrect" class="text-sm text-ink-secondary mt-0.5">
